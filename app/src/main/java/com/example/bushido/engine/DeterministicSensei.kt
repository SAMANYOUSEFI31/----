...
import com.example.bushido.data.model.DeterministicAutopsyResult
import com.example.bushido.data.model.DeterministicCourtResult
import com.example.bushido.data.model.DeterministicSenseiResult
import com.example.bushido.data.model.VulnerableHabit

object DeterministicSensei {

    fun getDeterministicAutopsy(
        ...
        if (failureReason == "دلایل شخصی") {
            return DeterministicAutopsyResult(
        ...
        var trap = "تله توهم کنترل زمان و انباشت اصطکاک‌های خرد"
        var analysis = "عدم مرزبندی مشخص میان ساعات تمرکز عمیق و فعالیت‌های پراکنده باعث فرسایش اراده شده است."
        var countermeasure = "قانون مقابله: مسدودسازی کلیه عوامل حواس‌پرتی تا پایان اجرای کار سخت روز."
        val tacticalActionTomorrow = "تعیین دقیق سنگین‌ترین وظیفه فردا روی کاغذ قبل از خواب امشب."
        ...
        if (missedHabits.isNotEmpty()) {
            analysis += " عدم اجرای «${missedHabits.joinToString("، ")}» مستقیماً انرژی ساختاری روز را تضعیف کرده است."
        }
        ...
        return DeterministicAutopsyResult(
            ...
            tacticalActionTomorrow = tacticalActionTomorrow
        )
    }

    fun getDeterministicSenseiAdvice(
        cycleTitle: String,
        elapsedDays: Int,
        remainingDays: Int,
        disciplinePercentage: Int,
        ...
        if (userQuery.isNotBlank()) {
            ...
            if (q.contains("صبح") || q.contains("سحرخیز") || q.contains("بیداری") || q.contains("خواب")) {
                return DeterministicSenseiResult(
            ...
            if (q.contains("ورزش") || q.contains("خستگی") || q.contains("انرژی") || q.contains("بدن")) {
                return DeterministicSenseiResult(
            ...
            if (q.contains("کار سخت") || q.contains("تمرکز") || q.contains("پشت گوش") || q.contains("اهمال") || q.contains("تنبلی")) {
                return DeterministicSenseiResult(
        ...
        if (disciplinePercentage >= 80) {
            ...
            strategicWarning = if (vulnerableHabits.isNotEmpty()) {
                "ضعف در ${vulnerableHabits.joinToString(" و ") { it.titleFa }} نیازمند تقویت فوری است."
            } else {
        ...
        return DeterministicSenseiResult(
            ...
            keyAdvice = keyAdvice,
            strategicWarning = strategicWarning,
            bushidoQuote = bushidoQuote
        )
    }

    fun getDeterministicCourtVerdict(
        cycleTitle: String,
        standardDays: Int,
        totalDays: Int,
        maxStreak: Int,
        ...
        if (disciplinePercentage >= 85) {
            ...
            strengths.addAll(listOf("تداوم بی‌نقص در زنجیره روزهای استاندارد", "مهار کامل وسوسه‌های اهمال‌کاری", "ایجاد پایداری حداکثری در ۵ رکن فونداسیون"))
            weaknesses.add("لزوم مراقبت از فرسودگی در دوره‌های با شدت بالا")
            ...
            strengths.addAll(listOf("پایداری عالی در شروع روز", "بازیابی موثر پس از روزهای افت", "کاهش نرخ روزهای سوخته بدون کالبدشکافی"))
            if (vulnerableHabits.isNotEmpty()) {
                weaknesses.add("افت مقطعی در رکن «${vulnerableHabits[0].titleFa}»")
            } else {
                weaknesses.add("نوسان در کار سخت در روزهای آخر هفته")
            }
            ...
            strengths.addAll(listOf("ثبت رکوردهای انفرادی در روزهای با انگیزه بالا", "پایبندی به تسویه بدهی‌های کالبدشکافی"))
            weaknesses.addAll(listOf("افت مکرر در فونداسیون‌های اصلی به ویژه در اواخر هفته", "پیوستگی ناکافی در زنجیره متوالی"))
            ...
            strengths.add("شجاعت در مواجهه با واقعیت داده‌ها و عدم انکار شکست")
            weaknesses.addAll(listOf("تسلیم شدن زودهنگام در برابر اصطکاک‌های خرد", "توقف‌های طولانی‌مدت پس از افت"))
            tacticalPlanForNextCycle = "آغاز فوری چرخه ترمیمی جدید با تمرکز صرف بر سحرخیزی و کار سخت به مدت ۳۰ روز."
        }

        return DeterministicCourtResult(
            verdict = verdict,
            grade = grade,
            senseiNotes = senseiNotes,
            strengths = strengths,
            weaknesses = weaknesses,
            tacticalPlanForNextCycle = tacticalPlanForNextCycle
        )
    }
}
